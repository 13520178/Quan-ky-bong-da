...
namespace BM_4
{
    public partial class Form4 : Form
    {
        ...
        SqlConnection con = new SqlConnection(@"Data Source=.\SQLEXPRESS;AttachDbFilename=|DataDirectory|\QL_BD.mdf;Integrated Security=True;Connect Timeout=30;User Instance=True");
        ...
        {
            // Khi click vào 1 cầu thủ trong danh sách:
            DateTime ngaysinh = new DateTime();
            if (SapXep.Rows.Count != 0)
            {
                ...
                for (int i = 0; i < thongtin.Rows.Count; i++)
                    if (dataGridView1.CurrentRow.Cells[1].Value.ToString().Replace(" ", "") == thongtin.Rows[i][7].ToString().Replace(" ", ""))
                    {
                        ...
                        richTextBox_NgaySinh.Text = ngaysinh.ToString("d");
        ...
        void Chon_ComboBox()
        {
            ...
            for(int i = 0; i < ten_soao.Rows.Count; i++)
            {
                ...
                if(!comboBox_SoAo.Items.Contains(ten_soao.Rows[i][1]))
            ...
            comboBox_SapXepTheo.Text = "Họ Tên";
        }
